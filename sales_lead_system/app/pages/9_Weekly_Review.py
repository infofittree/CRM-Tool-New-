...
page_header("Weekly Sales Review", "Executive weekly intelligence for review meetings.", "Weekly Performance")
...
ctrl2.markdown(f"<div style='background:var(--crm-card);border:1px solid var(--crm-border);border-radius:10px;padding:0.5rem 0.8rem;text-align:center;'><div style='font-size:0.7rem;color:var(--crm-muted);text-transform:uppercase;letter-spacing:0.3px;font-weight:700;'>Start</div><div style='font-size:1rem;font-weight:800;'>{start.strftime('%d %b %Y')}</div></div>", unsafe_allow_html=True)
ctrl3.markdown(f"<div style='background:var(--crm-card);border:1px solid var(--crm-border);border-radius:10px;padding:0.5rem 0.8rem;text-align:center;'><div style='font-size:0.7rem;color:var(--crm-muted);text-transform:uppercase;letter-spacing:0.3px;font-weight:700;'>End</div><div style='font-size:1rem;font-weight:800;'>{end.strftime('%d %b %Y')}</div></div>", unsafe_allow_html=True)
...
with db.session_scope() as session:
    ...
    tabs = st.tabs([
        ...
        "Lost Analysis", "Insights", "Next Week", "Present My Week", "Export",
    ])
    ...
    with tabs[0]:
        ...
        r1 = st.columns(5)
        r1[0].metric("New Leads", cur["assigned_week"])
        r1[1].metric("Contacted", cur["contacted_week"], fmt_delta("contacted_week"))
        r1[2].metric("Follow-ups Done", cur["followups_completed"], fmt_delta("followups_completed"))
        r1[3].metric("Meetings", cur["meetings_done"], fmt_delta("meetings_done"))
        r1[4].metric("Conversions", cur["conversions_week"], fmt_delta("conversions_week"))
        r2 = st.columns(5)
        ...
        st.divider()
        st.markdown("##### Pipeline Snapshot · Current")
        r3 = st.columns(5)
        ...
        r4 = st.columns(4)
        r4[0].metric("Pending F/U", cur["pending_followups"])
        r4[1].metric("Overdue F/U", cur["overdue_followups"])
        r4[2].metric("Conversion %", f"{cur['conversion_pct']}%")
        r4[3].metric("Hot / Warm", f"{cur['hot']} / {cur['warm']}")
    ...
    with tabs[1]:
        section_header("Salesperson Performance", "Ranked by lead-handling score.")
        ...
        for p in perf:
            ...
            with st.container():
                st.markdown(
                    ...
                    <span style="color:var(--crm-muted);font-weight:600">Handling {p['handling_score']} · Completion {p['completion_pct']}%</span></div>
    ...
    with tabs[2]:
        section_header("Companies Worked On", "Only leads with real activity — follow-ups, stage changes, notes.")
        rows = wr.company_activity(session, focus_user, start, end, only_worked=True)
        if not rows:
            empty_state("No work logged this week", "When the team logs follow-ups, those companies appear here.")
        else:
            ...
            data_table(view[display_cols])

    with tabs[3]:
        section_header("Company Deep Timeline", "Chronological week history for one company.")
        rows = wr.company_activity(session, focus_user, start, end, only_worked=True)
        if not rows:
            empty_state("No worked companies", "No companies were worked on this week.")
        else:
            ...
            if not tl:
                empty_state("No activity this week", "No logged actions for this company this week.")
            else:
                st.markdown("<div class='crm-timeline'>", unsafe_allow_html=True)
                for ev in tl:
                    st.markdown(
                        f"""<div class="crm-timeline-item">
                        <div class="crm-timeline-title">{ev['day']} · {ev['type']} <span style="color:var(--crm-muted)">({ev['source']})</span></div>
                        <div class="crm-timeline-meta">{ev['time']} · {ev['user']} — {ev['detail'] or 'No detail'}</div>
                        </div>""", unsafe_allow_html=True)
                st.markdown("</div>", unsafe_allow_html=True)

    with tabs[4]:
        section_header("Lost Opportunity Analysis", f"Deals marked Lost this week ({start:%d %b} → {end:%d %b}).")
        la = wr.lost_analysis(session, focus_user, start, end)
        if la["total_lost"] == 0:
            empty_state("No leads lost this week", "Leads marked Lost will appear here.")
        else:
            ...
            with c1:
                ...
                data_table(pd.DataFrame(la["by_person"]).fillna(0).T)
            with c2:
                st.caption("By Country")
                data_table(pd.DataFrame(la["by_country"]).fillna(0).T)
            st.caption("Lost companies")
            data_table(pd.DataFrame(la["rows"]))

    with tabs[5]:
        section_header("Management Insights", "Auto-generated business intelligence.")
    ...
    with tabs[6]:
        ...
        labels = {"hot_followup": "Hot/Warm Follow-ups", "negotiation": "Negotiation",
                  ...
                  "overdue": "Overdue", "high_nurture": "High-potential Nurture"}
        for key, label in labels.items():
            ...
            with st.expander(f"{label} ({len(data)})", expanded=(key in ("negotiation", "hot_followup"))):
                if not data:
                    ...
                    data_table(pd.DataFrame(data)[["company", "salesperson", "score", "band", "status", "next_followup"]])

    with tabs[7]:
        section_header("Present My Week", "Meeting-ready auto-summary.")
        ...
        if st.session_state.get("present_ready"):
            ...
            st.write(f"Contacted {ov['contacted_week']} companies · {ov['calls']} calls · {ov['whatsapp']} WhatsApp · "
                     f"{ov['emails']} emails · {ov['followups_completed']} follow-ups · {ov['conversions_week']} conversions.")
            pipe = wr.next_week_pipeline(session, focus_user)
            st.markdown("**2. Wins**")
            ...
            st.write(f"{ov['overdue_followups']} overdue · {ov['pending_followups']} pending.")
            st.markdown("**4. Hot Leads**")
            hot = pipe["hot_followup"][:8]
            data_table(pd.DataFrame(hot)[["company", "score", "band", "status"]] if hot else pd.DataFrame())
            st.markdown("**5. Negotiation Pipeline**")
            neg = pipe["negotiation"][:8]
            data_table(pd.DataFrame(neg)[["company", "score", "next_followup"]] if neg else pd.DataFrame())
            st.markdown("**6. Needs Help** (overdue + high score)")
            help_rows = [r for r in pipe["overdue"] if r["score"] >= 60][:8]
            data_table(pd.DataFrame(help_rows)[["company", "salesperson", "score", "status"]] if help_rows else pd.DataFrame())
    ...
    with tabs[8]:
        ...
        st.download_button("Download Full Weekly Report (Excel)", buffer.getvalue(),
        ...
        if not perf_df.empty:
            c2.download_button("Salesperson CSV", perf_df.to_csv(index=False),
